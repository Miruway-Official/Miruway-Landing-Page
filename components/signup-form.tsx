...
export function SignupForm({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "min-h-svh flex flex-col items-center justify-center px-6 md:px-10",
        className
      )}
      {...props}
    >
      {/* Large faded watermark behind the form */}
      <div
        aria-hidden
      ...
      {/* Form container — narrow, centered, on top of everything */}
      <div className="relative z-10 w-full max-w-[380px]">
        ...
        >
          {[
            ...
            <div key={id} className="space-y-2">
              <label
                htmlFor={id}
                className="text-[0.7rem] font-medium text-muted-foreground uppercase tracking-widest"
              >
                {label}
              </label>
              <Input
                ...
                required
                className="border-0 border-b border-border rounded-none bg-transparent px-0 py-2.5 text-foreground placeholder:text-muted-foreground/40 focus-visible:ring-0 focus-visible:border-primary transition-colors text-[0.95rem]"
              />
            </div>
          ))}
        </motion.div>
        ...
        >
          <Button
            type="submit"
            className="w-full h-12 font-semibold text-white rounded-md hover:opacity-90 active:scale-[0.98] transition-all text-[0.9rem]"
            style={{
        ...
        >
          {[
            ...
            {
              label: "Google",
              handler: () => authOAuthProvider({ provider: "google" }),
              icon: (
                <svg viewBox="0 0 24 24" className="w-4.5 h-4.5 fill-current">
            ...
            <button
              ...
              className="flex items-center justify-center h-11 rounded-md border border-border text-muted-foreground hover:border-primary/40 hover:text-foreground transition-colors"
            >
              {icon}
            </button>
          ))}
        ...
        <motion.p
          ...
          className="mt-10 text-xs text-muted-foreground/50 leading-relaxed text-center"
        >
